...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    // if (i == num_ints) {
  	...
  	if (i == num_ints) {
  	    double delta_time = Time_GetSeconds() - t;
        int delta_time_ms = delta_time * 1000;
        // printf("loop 0 in %.2f ms (appx. %d ms) (bandwidth: %.2f MB/s)\n",
    		//        1000 * delta_time, delta_time_ms,
        //        size_in_bytes / (1024.0*1024.0*delta_time));
        // printf("C prog about to return %d\n", delta_time_ms);
        printf("%d", delta_time_ms);
        // only printing the value so the Python script can read after redirecting stdout to file
        return 0;
  	}
  }
}

// set this up to work w/ mem_plot.py, but return value not getting through right.
